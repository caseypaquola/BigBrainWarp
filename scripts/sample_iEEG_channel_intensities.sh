...
rm -rf $bbwDir/spaces/bigbrainsym/iEEG_channels_${outName}_coord.txt
rm -rf $bbwDir/spaces/bigbrainsym/iEEG_channels_${outName}.txt
for ii in `seq 1 1 1772` ; do
	world_com=`mincstats -CoM -world_only -binvalue $ii $bbwDir/spaces/bigbrainsym/iEEG_channels_bigbrain.mnc`;
	worldtovoxel $sampleImage.mnc ${world_com##*: } >> $bbwDir/spaces/bigbrainsym/iEEG_channels_${outName}_coord.txt
done
mnc2nii $sampleImage.mnc $sampleImage.nii # convert to nifti so fslmeants can be used
for ii in `seq 1 1 1172` ; do
	vox=`sed "${ii}q;d" $bbwDir/spaces/bigbrainsym/iEEG_channels_${outName}_coord.txt`
	stringarray=($vox)
	...
	echo $int >> $bbwDir/spaces/bigbrainsym/iEEG_channels_${outName}.txt
done
