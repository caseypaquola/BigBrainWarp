function wrapper_mni2fsaverage(inputVol, interp, workDir, fileName, bbwDir, cbigDir)
...
%                      smooth data
% workDir 	 	working directory to save out data to 
% fileName 		name of output file (will be *h.fileName_fsaverage5.curv)
...
% read in volume
input = MRIread(char(inputVol));

% set extra variables
lh_map = [char(cbigDir) '/stable_projects/registration/Wu2017_RegistrationFusion/bin/' ...
        'final_warps_FS5.3/lh.avgMapping_allSub_RF_ANTs_MNI152_orig_to_fsaverage.mat'];
rh_map = [char(cbigDir) '/stable_projects/registration/Wu2017_RegistrationFusion/bin/' ...
...
write_curv([char(workDir) 'lh.' char(fileName) '_fsaverage5.curv'], lh_proj_data, 20480);
write_curv([char(workDir) 'rh.' char(fileName) '_fsaverage5.curv'], rh_proj_data, 20480);
